...
public class GuiSpawnUpgrade extends GuiScreen {
    ...
    protected ResourceLocation controls = new ResourceLocation("/extendedGUI/image/GUI Controls.png");
    float percentOut = 1;
    ...
    protected void actionPerformed(GuiButton par1GuiButton)
    {
    	if(par1GuiButton.id == this.radioButtons.id){
    		...
    		CommanderPacketHandeler.sendPacketToServer(upgradeable.generatePacket(units[radioButtons.selected], player));
    ...
    private void drawDetailsPanel() {
        ...
        if(width > 205 + 2*(110) && getSelectedUnit() != null){
	    	GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
	    	this.mc.renderEngine.bindTexture(controls);
	    	...
	    	//Left Panel Text
	    	String title = StatCollector.translateToLocal(getSelectedUnit().getUnitName()) + " "+ StatCollector.translateToLocal("gui.details.stats");

            fontRendererObj.drawString(title, (int)(110*percentOut) - (125 + fontRendererObj.getStringWidth(title))/2, guiTop + 8, 0x404040);
	    	
	    	drawString(fontRendererObj, StatCollector.translateToLocal("gui.details.attack")+": "+getSelectedUnit().getAttackStrength(),
	    			3, guiTop+26, 0xFFFFFF);
	    	drawString(fontRendererObj, StatCollector.translateToLocal("gui.details.health")+": "+getSelectedUnit().getMaxHealth(),
	    			3, guiTop+26+12, 0xFFFFFF);
	    	drawString(fontRendererObj, StatCollector.translateToLocal("gui.details.armour")+": "+getSelectedUnit().getBaseArmour(),
	    			3, guiTop+26+24, 0xFFFFFF);
	    	
	    	if(units[radioButtons.selected] instanceof IRangedAttackMob){
		    	drawString(fontRendererObj, StatCollector.translateToLocal("gui.details.range")+": "+getSelectedUnit().getTotalArmorValue(),
		    			3, guiTop+26+36, 0xFFFFFF);
		    	drawString(fontRendererObj, StatCollector.translateToLocal("gui.details.rate")+": "+getSelectedUnit().getTotalArmorValue(),
	    	...
	    	if(abilities.length > 0){
		    	drawCenteredString(fontRendererObj, StatCollector.translateToLocal("gui.details.special"), 110/2, guiTop+26+65, 0xFFFF00);

		    	for(int i = 0; i < abilities.length; i++){
		    		if(abilities[i].isPositive())
		    			drawString(fontRendererObj, "+", 3, guiTop+26+65+14+i*12, 0x00FF00);
			    	else
			    		drawString(fontRendererObj, "-", 3, guiTop+26+65+14+i*12, 0xFF0000);
		    		
		    		drawString(fontRendererObj, StatCollector.translateToLocal(abilities[i].getTranslatedName(-1)),
		    				10, guiTop+26+65+14+i*12, 0xFFFFFF);
		    	}
	    	}
	    	
	    	//Right Panel Text
	    	title = StatCollector.translateToLocal(getSelectedUnit().getUnitName());

            fontRendererObj.drawString(title, (int)(width - 110*percentOut) + 55 - fontRendererObj.getStringWidth(title)/2 , guiTop + 8, 0x404040);
	    	
	    	
	    	String desc = MBClientHelper.parseUnitDescription(getSelectedUnit());
            fontRendererObj.drawSplitString(desc, width - (int)(110*percentOut) + 11, guiTop+26, 90, (0xFFFFFF& 16579836) >> 2 | 0xFFFFFF & -16777216);
            fontRendererObj.drawSplitString(desc, width - (int)(110*percentOut) + 10, guiTop+25, 90, 0xFFFFFF);
