...
public class GUIOrderMenu extends Gui {
    ...
    public void renderOverlayRightBox()
    {
    	...
    	if(!isHidden()){
            ...
            String message = StatCollector.translateToLocal("gui.order.label")+" ("+MBCommander.PROXY.getSelectedUnits().size()+")";
    	...
    	if(selected >= 0 && selected < menu.length){
    		...
    		try{
				...
				case StandGuard:
                    ...
                    buf.writeByte((byte)orders[selected].ordinal());
                    buf.writeByte((byte)units.size());
					for (EntityMBUnit entityMBUnit : units) {
                        buf.writeInt(entityMBUnit.entityId);
					}
					
					packet = new FMLProxyPacket(buf, CommanderPacketHandeler.orderPacket);
		    	...
		    	if(packet != null){
		    		CommanderPacketHandeler.sendPacketToServer(packet);
		    	}
